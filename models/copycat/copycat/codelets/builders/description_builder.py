...
class DescriptionBuilder(Builder):
    ...
    def run(self, temperature: float) -> CodeletResult:
        self.argument_object = self.proposed_description.argument_object
        if self.argument_object not in self.workspace.objects:
            return Fizzle(FizzleReason.OBJECTS_NO_LONGER_EXIST)
        self.descriptor = self.proposed_description.descriptor
        self.facet = self.proposed_description.facet
        self.slipnet.activate_node_from_workspace(self.descriptor.name)
        self.slipnet.activate_node_from_workspace(self.facet.name)
        if self.argument_object.has_description(self.proposed_description):
            return Fizzle(FizzleReason.STRUCTURE_ALREADY_EXISTS)
        self.argument_object.add_description(self.proposed_description)
        return Finish()
