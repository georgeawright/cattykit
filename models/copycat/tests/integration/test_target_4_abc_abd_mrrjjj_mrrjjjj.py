...
def test_single_run(monkeypatch):
    ...
    for codelet_index, (first_letter, number_of_bonds) in enumerate(
        ...
        monkeypatch.setattr(
            ...
            lambda *_, target_string=copycat.workspace.target_string: target_string,
        )
        monkeypatch.setattr(
            copycat.workspace.target_string,
            "choose_object",
            lambda *_, first_letter=first_letter: first_letter,
        )
        monkeypatch.setattr(selected_codelet[0], "_choose_direction", lambda *_: None)
        monkeypatch.setattr(
            selected_codelet[0],
            "_choose_number_of_bonds",
            lambda *_, number_of_bonds=number_of_bonds: number_of_bonds,
        )
        monkeypatch.setattr(
            selected_codelet[0],
            "_get_first_bond",
            lambda *_, first_letter=first_letter: first_letter.right_bond,
        )
        copycat.coderack.post(selected_codelet[0], temperature=0.0)
        codelet = copycat.coderack.choose(temperature=0.0)
        assert codelet is selected_codelet[0]
    ...
    monkeypatch.setattr(copycat.workspace.target_string, "choose_object", lambda *_: m)
    monkeypatch.setattr(selected_codelet[0], "_choose_direction", lambda *_: None)
    monkeypatch.setattr(selected_codelet[0], "_choose_number_of_bonds", lambda *_: 0)
    monkeypatch.setattr(selected_codelet[0], "_get_first_bond", lambda *_: None)
    copycat.coderack.post(selected_codelet[0], temperature=0.0)
    codelet = copycat.coderack.choose(temperature=0.0)
    assert codelet is selected_codelet[0]
    assert codelet.run(temperature=0.0) == Finish()
    ...
    for codelet_index, (source_group, target_group) in enumerate(
        ...
        monkeypatch.setattr(
            copycat.workspace,
            "choose_object",
            lambda *_, source_group=source_group: source_group,
        )
        monkeypatch.setattr(
            source_group,
            "choose_neighbor",
            lambda *_, target_group=target_group: target_group,
            raising=False,
        )
        monkeypatch.setattr(
            selected_codelet[0],
            "_choose_bond_facet",
            lambda *_: copycat.slipnet["length"],
        )
        monkeypatch.setattr(
            selected_codelet[0],
            "_get_bond_category",
        ...
        assert codelet is selected_codelet[0]
        assert codelet.run(temperature=0.0) == Finish()
        assert copycat.coderack.number_of_codelets_run == 110 + codelet_index * 3
        assert (
        ...
        assert codelet is selected_codelet[0]
        assert codelet.run(temperature=0.0) == Finish()
        assert copycat.coderack.number_of_codelets_run == 111 + codelet_index * 3
        assert any(
            isinstance(posted, BondBuilder) for posted in copycat.coderack.codelets
        )
        selected_codelet[0] = [
        ...
        assert codelet is selected_codelet[0]
        assert codelet.run(temperature=0.0) == Finish()
        assert copycat.coderack.number_of_codelets_run == 112 + codelet_index * 3
        assert (
