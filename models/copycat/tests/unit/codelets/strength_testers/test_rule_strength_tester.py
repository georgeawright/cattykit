...
def test_run():
    ...
    proposed_rule = Mock()
    strength_tester = RuleStrengthTester(
        urgency_bin=0,
        coderack=coderack,
        slipnet=slipnet,
        workspace=workspace,
    ...
    # weak rule should not post a builder
    proposed_rule.total_strength = 0.0
    ...
    # strong rule should post a builder
    slipnet.activate_called = 0
    coderack.post_called = 0
    proposed_rule.total_strength = 1.0
    result = strength_tester.run(temperature=0.0)
