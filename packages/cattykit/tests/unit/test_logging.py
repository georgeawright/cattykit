...
from cattykit.logging import NullLogger, PrintLogger, CattycamLogger
...
def test_cattycam_logger_creates_the_cattycam_schema(tmp_path) -> None:
...
def test_cattycam_logger_populates_cattycam_run_history(tmp_path, monkeypatch) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")
    monkeypatch.setattr(
        "cattykit.logging.cattycam_logger.perf_counter_ns", lambda: 10_000_000_000
    )
    ...
    monkeypatch.setattr(
        "cattykit.logging.cattycam_logger.perf_counter_ns", lambda: 10_125_000_000
    )
    logger.log(
        "codelet_finished", codelet_id="c-1", outcome="fizzle", reason="no_bond", time=1
    )
...
def test_cattycam_logger_records_a_posted_codelets_proposed_structure(tmp_path) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")
    ...
    with sqlite3.connect(database) as connection:
        columns = {row[1] for row in connection.execute("PRAGMA table_info(codelets)")}
...
def test_cattycam_logger_excludes_codelet_step_logging_time_from_time_taken(
    ...
    monkeypatch.setattr(
        "cattykit.logging.cattycam_logger.perf_counter_ns", lambda: next(timestamps)
    )
...
def test_cattycam_logger_records_git_metadata_for_each_run(
    tmp_path, monkeypatch
) -> None:
    database = tmp_path / "cattycam.sqlite"
    monkeypatch.setattr(
...
def test_cattycam_logger_attaches_its_codelet_time_to_untimed_events(tmp_path) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")

    logger.log("run_started", problem="abc -> abd")
    logger.log(
        "attribute_updated", object_id="temperature", attribute="value", value=100.0
    )
    logger.log("codelet_selected", codelet_id="c-1", codelet_type="BottomUpBondScout")
    logger.log(
        "attribute_updated", object_id="temperature", attribute="value", value=90.0
    )
    logger.close()
...
def test_cattycam_logger_records_ordered_codelet_steps(tmp_path) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")
    logger.log("run_started", problem="abc -> abd")
...
def test_cattycam_logger_records_translated_rules_separately(tmp_path) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")
    logger.log("run_started", problem="abc -> abd")
    logger.log(
...
def test_cattycam_logger_updates_a_posted_codelet_when_selected(tmp_path) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")
    logger.log("run_started", problem="abc -> abd")
    logger.log(
...
def test_cattycam_logger_assigns_selected_codelet_as_parent_of_new_posts(
    tmp_path,
) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")
...
def test_cattycam_logger_populates_typed_workspace_tables(tmp_path) -> None:
    database = tmp_path / "cattycam.sqlite"
    logger = CattycamLogger(database, "copycat")
    logger.log("run_started", problem="abc -> abd")
    logger.log(
